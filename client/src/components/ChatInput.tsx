import { MouseEvent, useState, FormEvent, KeyboardEvent, ChangeEvent } from "react";
...
function ChatInput({ handleSendMessage }: Props) {
  ...
  const onEnterSendChat = (event: KeyboardEvent) => {
    if (message.length > 0) {
      if (event.key === "13") {
        handleSendMessage(message);
        setMessage("");
